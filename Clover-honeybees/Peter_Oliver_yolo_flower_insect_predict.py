...
def main():
    ...
    Returns:
        None. Saves prediction results in the specified output directory as summary CSV file.
    """
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        ...
        for img_name in image_files:
            ...
            # Save results to txt file (YOLO format: class x1 y1 x2 y2 flower_conf insect_class insect_conf)
            ...
            with open(out_path, 'w') as f:
                ...
                for i, (x1, y1, x2, y2, score, cls) in enumerate(coords):
                    ...
                    if confidence > 0.19 and predicted_class in [498,501,504]:
                        num_insects += 1
                        # save the cropped image
                        cutout_path = os.path.join(OUTPUT_DIR, f"{os.path.splitext(img_name)[0]}_flower{i}_insect{num_insects}.jpg")
                        cutouts[i].save(cutout_path)
                    # Write flower detection and insect prediction
                    f.write(f"{int(cls)} {x1} {y1} {x2} {y2} {score:.4f} {predicted_class} {confidence:.4f}\n")

            # Parse datetime from filename
            dt_str = parse_datetime_from_filename(img_name)
            # Write summary row for this image
            csvwriter.writerow([img_name, dt_str, num_flowers, num_insects])
